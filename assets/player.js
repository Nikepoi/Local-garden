...
async function init(){
        ...
        try { 
            if (document.fullscreenElement) {
                ...
                if (videoWidth > 0 && videoHeight > 0) {
                    ...
                    dbg('Fullscreen: Aspect ratio set:', `${videoWidth}/${videoHeight}`);
                } else {
                    dbg('Fullscreen: Metadata not available, using default aspect ratio');
                }
            }
    ...
    if (cinemaBtn) cinemaBtn.addEventListener('click', () => { 
        ...
        if (videoWidth > 0 && videoHeight > 0) {
            playerWrap.style.aspectRatio = `${videoWidth}/${videoHeight}`;
            dbg('Theater: Aspect ratio set:', `${videoWidth}/${videoHeight}`);
        } else {
            dbg('Theater: Metadata not available, using default aspect ratio');
        }
    });
