...
/* --- Inject small CSS for download list and video player --- */
(function injectDownloadCss(){
    const css = `
        ...
        .download-link.item { display:flex; align-items:center; justify-content:space-between; background:#000; color:#fff; padding:14px; border-radius:12px; text-decoration:none; font-weight:700; gap:8px; box-shadow: 0 6px 14px rgba(0,0,0,0.12); }
        ...
        .theater #playerWrap, .theater #videoWrap {
            width: 100%;
            height: 100vh;
            max-height: 100vh;
            display: flex;
        ...
        :fullscreen #playerWrap, :fullscreen #videoWrap {
            width: 100%;
            height: 100%;
            display: flex;
            justify-content: center;
...
async function init(){
    ...
    if (!player) { 
        ...
        errDiv.textContent = 'Error: Video player tidak ditemukan di halaman.';
        document.body.appendChild(errDiv);
        dbg('no <video id="player"> element found'); 
        return; 
    }
    ...
    const progress = document.getElementById('progress') || (function(){ 
        const el=document.createElement('input'); 
        el.type='range'; el.id='progress'; el.min=0; el.max=100; el.value=0; el.className='vp-progress'; 
        if (playerWrap) playerWrap.appendChild(el); 
        return el; 
    })();
    const timeEl = document.getElementById('time') || (function(){ 
        const el=document.createElement('div'); 
        el.id='time'; el.className='vp-time'; 
        if (playerWrap) playerWrap.appendChild(el); 
        return el; 
    })();
    const playBtn = document.getElementById('playPause') || (function(){ 
        const b=document.createElement('button'); 
        b.id='playPause'; b.className='icon-btn'; 
        if (playerWrap) (playerWrap.querySelector('.buttons-row')||playerWrap).appendChild(b); 
        return b; 
    })();
    const bigPlay = document.getElementById('bigPlay') || (function(){ 
        const b=document.createElement('button'); 
        b.id='bigPlay'; b.className='big-play-btn'; 
        b.textContent='Play'; 
        (playerWrap || document.body).appendChild(b); 
        return b; 
    })();
    let iconPlay = document.getElementById('iconPlay');
    if (!iconPlay) {
        iconPlay = document.createElementNS('http://www.w3.org/2000/svg','svg');
        iconPlay.setAttribute('id','iconPlay'); 
        iconPlay.setAttribute('viewBox','0 0 24 24'); 
        iconPlay.setAttribute('width','18'); 
        iconPlay.setAttribute('height','18');
    ...
    const muteBtn = document.getElementById('mute') || (function(){ 
        const b=document.createElement('button'); 
        b.id='mute'; b.className='icon-btn'; 
        b.textContent='🔊'; 
        if (playerWrap) (playerWrap.querySelector('.buttons-row')||playerWrap).appendChild(b); 
        return b; 
    })();
    ...
    const downloadArea = document.getElementById('downloadArea') || (function(){ 
        const d=document.createElement('div'); 
        d.id='downloadArea'; 
        document.querySelector('.post-actions')?.appendChild(d); 
        return d; 
    })();
    ...
    const volumeIndicator = document.getElementById('volumeIndicator') || (function(){ 
        const el=document.createElement('div'); 
        el.id='volumeIndicator'; 
        el.className='volume-indicator'; 
    ...
    try {
        ...
        const el = document.getElementById('debug'); 
        if (el) el.textContent = 'posts.json load failed: ' + String(e);
        return;
    }
    ...
    if (!post) {
        ...
        const el = document.getElementById('debug'); 
        if (el) el.textContent = 'Tidak menemukan post untuk slug: ' + slugUsed;
        return;
    }
    ...
    if (postTitle) { 
        ...
        if (!postTitle.parentElement) document.getElementById('metaBox')?.appendChild(postTitle); 
    }
    if (postDate) { 
        postDate.textContent = post.date || ''; 
        if (!postDate.parentElement) document.getElementById('metaBox')?.appendChild(postDate); 
    }
    if (postDesc) { 
        postDesc.innerHTML = (post.description || post.excerpt || '').toString().replace(/<img\b[^>]*>/gi,'').replace(/\n/g,'<br>'); 
        if (!postDesc.parentElement) document.getElementById('metaBox')?.appendChild(postDesc); 
    }
    ...
    async function setupMediaForUrl(u){
                ...
                if (window.Hls && Hls.isSupported()){
                    ...
                    hls.on(Hls.Events.MANIFEST_PARSED, (event, data) => {
                        ...
                        if (levels.length) {
                            ...
                            if (width > 0 && height > 0) {
                                const playerWrap = document.getElementById('playerWrap') || player.parentElement;
                                if (playerWrap) playerWrap.style.aspectRatio = `${width}/${height}`;
                                dbg('HLS video aspect ratio set:', `${width}/${height}`);
                            }
                        }
                    });
                    dbg('HLS attached', src);
                } else {
                    const s = document.createElement('source'); 
                    s.src = src; 
        ...
        if (playerWrap) {
            ...
            dbg('Container size:', `${wrapRect.width}x${wrapRect.height}`);
            dbg('Video element size:', `${playerRect.width}x${playerRect.height}`);
            if (player.videoWidth && player.videoHeight) {
                dbg('Video native resolution:', `${player.videoWidth}x${player.videoHeight}`);
            }
        }
        player.addEventListener('loadedmetadata', () => {
            ...
            if (videoWidth > 0 && videoHeight > 0) {
                const playerWrap = document.getElementById('playerWrap') || player.parentElement;
                if (playerWrap) playerWrap.style.aspectRatio = `${videoWidth}/${videoHeight}`;
                dbg('Video aspect ratio set:', `${videoWidth}/${videoHeight}`);
            }
        }, { once: true });
    }
            ...
            try {
                ...
                if (debugEl) {
                    debugEl.innerHTML = [
                        ...
                        ' — Playback gagal. Pastikan:',
                        '<br>1. URL video valid: ' + (currentSrc || '[kosong]'),
                        '<br>2. Server mendukung Content-Type: video/mp4',
                        '<br>3. Interaksi pengguna diperlukan untuk autoplay.',
                        '<br>Error: ' + (player.error ? JSON.stringify(player.error) : 'null'),
                        '<br>Network: ' + ns,
                        '<br>Ready: ' + rs
                    ].join('');
                }
    ...
    if (muteBtn) muteBtn.addEventListener('click', (ev)=> {
        ...
        updateMuteUI(); 
        showVolumeIndicator(Math.round((player.muted?0:player.volume)*100));
    ...
    function showVolumeIndicator(perc){ 
        if (!volumeIndicator) return; 
        volumeIndicator.style.display='inline-flex'; 
        volumeIndicator.textContent = `Volume ${perc}%`; 
        if (window._volTimeout) clearTimeout(window._volTimeout); 
        window._volTimeout = setTimeout(()=> volumeIndicator.style.display = 'none', 900); 
    }
        ...
        try { 
            if (document.fullscreenElement) {
                ...
                if (videoWidth > 0 && videoHeight > 0) {
                    ...
                    dbg('Fullscreen: Metadata belum load, skip set aspectRatio');
                }
            }
    ...
    if (cinemaBtn) cinemaBtn.addEventListener('click', () => { 
        ...
        const videoHeight = player.videoHeight;
        if (videoWidth > 0 && videoHeight > 0) {
            playerWrap.style.aspectRatio = `${videoWidth}/${videoHeight}`;
        } else {
            dbg('Theater: Metadata belum load, skip set aspectRatio');
        }
    });
    ...
    document.addEventListener('keydown', (e)=> {
        ...
        if (e.key === 'ArrowUp'){ 
            ...
            showVolumeIndicator(Math.round(player.volume*100)); 
            volSlider && (volSlider.value = Math.round(player.volume*100)); 
            updateMuteUI(); 
        }
        if (e.key === 'ArrowDown'){ 
    ...
    (function attachPlaylist(){
        if (!Array.isArray(post._streams) || post._streams.length <= 1) return;
        const streams = post._streams.slice();
        const old = document.getElementById('playlistControls'); 
        if (old) old.remove();
        const plc = document.createElement('div'); 
        plc.id='playlistControls'; 
        plc.className='playlist-controls'; 
        ...
        idxInput.addEventListener('change', async () => { 
            const v = Number(idxInput.value)-1; 
            if (Number.isInteger(v) && v>=0 && v<streams.length) await playAt(v); 
...
init().catch(err => { 
    ...
    try { 
        const el=document.getElementById('debug'); 
        if (el) el.textContent = 'init error: '+String(err); 
    } catch(e){} 
});
})();
